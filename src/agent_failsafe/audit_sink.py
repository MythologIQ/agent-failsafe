...
def _sign_record(
    ...
    content = _content_for_hash(record)
...
def verify_audit_integrity(
    ...
    """Verify hash chain integrity of an audit ledger.

    Standalone function that opens a separate read-only connection.
    """
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute("SELECT * FROM audit_entries ORDER BY id ASC")
        prev_hash = "0" * 64
        for row in cursor.fetchall():
            content = _content_for_hash(dict(row))
            expected_hash = hashlib.sha256(f"{prev_hash}:{content}".encode()).hexdigest()
            if row["entry_hash"] != expected_hash:
                conn.close()
                return False, (
                    f"Hash mismatch at entry {row['id']}: "
                    f"expected {expected_hash[:16]}..., got {row['entry_hash'][:16]}..."
                )
            expected_sig = hmac.new(hmac_key, expected_hash.encode(), hashlib.sha256).hexdigest()
...
_INSERT_SQL = """INSERT INTO audit_entries
...
class FailSafeAuditSink:
    """HMAC-signed, Merkle-chained audit sink for the FailSafe ledger.

    Implements ``agentmesh.governance.audit_backends.AuditSink`` protocol.
    """
    ...
    def __init__(
        ...
        if hmac_key is None:
            raise ValueError(
                "hmac_key is required. Set a production key or use "
                "b'failsafe-dev-key' explicitly for development."
            )
        ...
        self._conn = sqlite3.connect(str(self.ledger_path))
        self._conn.execute("PRAGMA journal_mode=WAL")
        self._conn.execute("PRAGMA busy_timeout=5000")
        self._ensure_table()

    def __repr__(self) -> str:
        return f"FailSafeAuditSink(ledger_path={self.ledger_path!r})"
    ...
    def verify_integrity(self) -> tuple[bool, str | None]:
        ...
        return verify_audit_integrity(self.ledger_path, self._hmac_key)
    ...
    def _write_entry(self, entry: Any) -> None:
        ...
        entry_hash, signature = _sign_record(record, self._prev_hash, self._hmac_key)
